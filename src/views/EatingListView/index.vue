...
const initData = ref(DEFAULT_DATA);
...
const todoList = computed(() => initData.value.filter(({ food }) => food));
const doneList = computed(() => initData.value.filter(({ food }) => !food));
...
<template>
    <main @click="close" class="main">
        ...
        <section>
            <h2>昨日摄入 <span id="todocount">{{ todoList.length }}</span></h2>
            <ol id="eatlist" class="demo-box">
                <li v-for="item in todoList" :key="item.id" @click="({ target }) => tipOpen(item.id, target)">
                    <span class="heat">1020</span>
            ...
            <h2>今日消耗 <span id="donecount">{{ doneList.length }}</span></h2>
            <ul id="hotlist">
                <li v-for="item in doneList" :key="item.id" @click="({ target }) => tipOpen(item.id, target)">
                    <span class="heat">300</span>
        ...
        <div class="popCount nodie" v-if="tipsVisible">
            <div class="pop">
                <dl class="steps">
                    ...
                    <!-- <dd><span class="label">摄入:</span>
                        <IconC value="&#xe666;" color="orange" />230 Cal
                    </dd>
                    <dd><span class="label">时间:</span>
                        <IconC value="&#xe614;" color="orange" />12点25分
                    </dd>
                    <dd><span class="label">价格:</span>
                        <IconC value="&#xe60b;" color="orange" />60元
                    </dd>
                    <dd><span class="label">心情:</span>
                        <IconC value="&#xe616;" color="orange" />还行
                    </dd> -->


                    <dd><span class="label">消耗:</span>
                        <IconC value="&#xe671;" color="orange" />
                        230 Cal
                    </dd>
                    <dd><span class="label">时间:</span>
                        <IconC value="&#xe63f;" color="orange" />
...
#eatlist {
    span.heat {
        color: var(--hot9-color);
    }

    li {
        border-left: 5px solid var(--eat-color);
    }

    li:hover {
        background-color: var(--eat9-color);
    }
}

#hotlist {
    span.heat {
        color: var(--eat9-color);
    }
    ...
    li:hover {
        background-color: var(--hot9-color);
    }
}
...
/* //at.alicdn.com/t/c/font_4469007_5ven5iec8bx.js */
